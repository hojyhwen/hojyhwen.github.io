...
var dealOne = function() {
    var r = rand(1, 52)
    var img0 = $('.col-2>img')[0]
    var $img0 = $(img0)
    $img0.attr('src', './poker/pic' + r + '.png')
}

var dealFive = function() {
        //產生52張牌
        var allpoker = [];
        for (var i = 1; i <= 52; i++) { allpoker.push(i) }
        //洗牌
        for (var i = 1; i <= 100; i++) {
            var r = rand(1, 52) - 1

            var temp = allpoker[0]
            allpoker[0] = allpoker[r]
            allpoker[r] = temp
        }
...
//     }
//i是畫面五張牌 v是數字
var changeCard = function(i, v) {
    var img = $('.col-2>img')[i]
    var $img = $(img)
    $img.attr('src', './poker/pic' + v + '.png')
    $img.attr('data-card', v);
    $img.attr('data-value', Math.ceil(v / 4));
    $img.attr('data-color', (v - 1) % 4);

    $img.on('click', function() {
        var Color = ['梅花', '方塊', '愛心', '黑桃']
        var value = $(this).attr('data-value')
        var color = $(this).attr('data-color')
        console.log("這張是" + Color[color] + '' + value)
            //     $(this).attr('src', './poker/pic' + v + '.png');
        if (value < 10) { $(this).hide(); }

    })
}

$('#deal').on('click', dealFive);

// $('#deal2').on('click', dealtwo);
// var card = [];
